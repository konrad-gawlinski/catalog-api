...
class Migrator
{
  ...
  function initializeProductRelations()
  {
    ...
    while ($row = pg_fetch_assoc($products)) {
      pg_query("SELECT catalog_sp.ct_create_node('{$row['id']}');");
    }
  }
  ...
  function createConfigBundleRelations()
  {
    $products = pg_query($this->dbCon,
      "SELECT b.product_ida as config_id, b.product_idb as bundle_id,
              p1.sku as config_sku, p2.sku as bundle_sku,
              e1.id as config_entity, e2.id as bundle_entity 
